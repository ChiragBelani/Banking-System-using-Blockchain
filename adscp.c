...
void addBlock(char *data) {
    ...
    computeHash(data, newBlock->hash); //hold1
    newBlock->next = NULL;
...
void menu() {
    BankDatabase db;
    initBankDatabase(&db);
    loadUsersFromCSV(&db, "C:\\Banking System using Blockchain\\users.csv");
    loadTransactionsFromCSV("C:\\Banking System using Blockchain\\transactions.csv");
    ...
    do {
        ...
        switch (choice) {
            case 1:
                ...
                scanf("%s", mobile); // 
                if (strlen(mobile) != 10 && !isNumeric(mobile)) {
                ...
                if (user) {
                    ...
                    addBlock(data);            
                    saveUsersToCSV(&db, "C:\\Banking System using Blockchain\\users.csv");
                    saveTransactionsToCSV("C:\\Banking System using Blockchain\\transactions.csv");
